...
def setup(db, user, other_user):
    agency = agency_approve(agency=agency_create(user=user, name="Agency A", description="d"))
    admin = AgencyEmployee.objects.get(user=user, agency=agency)

    cheap = _make_tour(agency, admin, title="Budget Rome", price=Decimal("200.00"),
                       location_data={"country": "Italy", "city": "Rome"},
                       start_date=date(2026, 6, 1), end_date=date(2026, 6, 4))
    mid = _make_tour(agency, admin, title="Paris Adventure", price=Decimal("600.00"),
                     location_data={"country": "France", "city": "Paris"},
                     start_date=date(2026, 7, 1), end_date=date(2026, 7, 8))
    expensive = _make_tour(agency, admin, title="Luxury Spain", price=Decimal("1500.00"),
...
class TestTourOrdering:
    ...
    def test_order_by_start_date_asc(self, api_client, setup):
        agency, cheap, mid, expensive = setup
        response = api_client.get(_list_url(agency.pk), {"ordering": "start_date"})
    ...
    def test_order_by_duration_desc(self, api_client, setup):
        agency, cheap, mid, expensive = setup
        response = api_client.get(_list_url(agency.pk), {"ordering": "-duration_days"})
