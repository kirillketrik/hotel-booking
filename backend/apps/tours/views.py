...
from apps.tours.permissions import IsAgencyAdmin, IsAgencyMember
...
class AgencyViewSet(
    ...
    def partial_update_agency(self, request, pk=None):
        agency = get_object_or_404(
            Agency, pk=pk, employees__user=request.user
        )
...
class TourViewSet(
    ...
    search_fields = ["title", "description", "location__country", "location__city"]
    ordering_fields = ["price", "start_date", "created_at", "duration_days"]
    ordering = ["-created_at"]

    def get_queryset(self):
        user = self.request.user
        approved_agency_only = not (
            user.is_authenticated and user.is_staff
        )
        agency_pk = self.kwargs.get("agency_pk")
        if agency_pk:
            return tour_list(agency=agency_pk, approved_agency_only=approved_agency_only)
