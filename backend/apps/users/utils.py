...
def _set_auth_cookie(
    response: Response, key: str, value: str, max_age: timedelta
):
    response.set_cookie(
        key=key,
        value=value,
...
def set_access_cookie(response: Response, value: str):
    _set_auth_cookie(
        ...
        value=value,
        max_age=timedelta(
            minutes=settings.SETTINGS.AUTH.ACCESS_TOKEN_LIFETIME
        ),
    )
...
def set_refresh_cookie(response: Response, value: str):
    _set_auth_cookie(
        ...
        value=value,
        max_age=timedelta(
            minutes=settings.SETTINGS.AUTH.REFRESH_TOKEN_LIFETIME
        ),
    )
