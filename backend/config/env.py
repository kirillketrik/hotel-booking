...
class SecuritySettings(BaseSettings):
    SECRET_KEY: SecretStr = Field(SecretStr("VERY_VERY_SElRET_KEY"))
    DB_ENCRYPTION_KEY: SecretStr = Field(
        SecretStr("lSu3tUddJ-shn9h334QDaxSyahx0WE6a_brZTKjw8KQ=")
    )
...
class DatabaseSettings(BaseSettings):
    ...
    USER: SecretStr = Field(
        SecretStr("admin"), validation_alias="POSTGRES_USER"
    )
    PASSWORD: SecretStr = Field(
        SecretStr("admin"), validation_alias="POSTGRES_PASSWORD"
    )
...
class MinioSettings(BaseSettings):
    ENDPOINT: str = Field(
        "minio:9000", validation_alias="MINIO_STORAGE_ENDPOINT"
    )
    EXTERNAL_URL: str = Field(
        "http://localhost:9000", validation_alias="MINIO_EXTERNAL_URL"
    )
    ACCESS_KEY: SecretStr = Field(
        SecretStr("admin"), validation_alias="MINIO_STORAGE_ACCESS_KEY"
    )
    SECRET_KEY: SecretStr = Field(
        SecretStr("admin"), validation_alias="MINIO_STORAGE_SECRET_KEY"
    )
    USE_HTTPS: bool = Field(False, validation_alias="MINIO_STORAGE_USE_HTTPS")
    MEDIA_BUCKET_NAME: str = Field(
        "django-media", validation_alias="MINIO_STORAGE_MEDIA_BUCKET_NAME"
    )
    STATIC_BUCKET_NAME: str = Field(
        "django-static", validation_alias="MINIO_STORAGE_STATIC_BUCKET_NAME"
    )
    AUTO_CREATE_MEDIA_BUCKET: bool = Field(
        True, validation_alias="MINIO_STORAGE_AUTO_CREATE_MEDIA_BUCKET"
    )
    AUTO_CREATE_STATIC_BUCKET: bool = Field(
        True, validation_alias="MINIO_STORAGE_AUTO_CREATE_STATIC_BUCKET"
    )
    STATIC_USE_PRESIGN: bool = Field(
        False, validation_alias="MINIO_STORAGE_STATIC_USE_PRESIGN"
    )
    MEDIA_USE_PRESIGN: bool = Field(
        False, validation_alias="MINIO_STORAGE_MEDIA_USE_PRESIGN"
    )


class AuthSettings(BaseSettings):
    JWT_SECRET_KEY: SecretStr = Field(
        SecretStr("yeyeyey"), validation_alias="JWT_SECRET_KEY"
    )
    ACCESS_TOKEN_LIFETIME: int = Field(
        60, validation_alias="ACCESS_TOKEN_LIFETIME"
    )
    REFRESH_TOKEN_LIFETIME: int = Field(
        1440, validation_alias="REFRESH_TOKEN_LIFETIME"
    )
    ...
    AUTH_COOKIE_SECURE: bool = Field(
        False, description="Should be set to True in production"
    )
    ...
    def DJANGO_DICT(self) -> Dict[str, str]:
        return {
            "ACCESS_TOKEN_LIFETIME": timedelta(
                minutes=self.ACCESS_TOKEN_LIFETIME
            ),
            "REFRESH_TOKEN_LIFETIME": timedelta(
                minutes=self.REFRESH_TOKEN_LIFETIME
            ),
