...
import type { TourReview } from '@/lib/types'
...
function ReviewForm({
  ...
  const mutation = useMutation({
    mutationFn: () =>
      isEdit
        ? apiEndpoints.reviews.update(tourId, reviewId!, { rating, comment })
        : apiEndpoints.reviews.create(tourId, { rating, comment }),
    onSuccess: () => {
...
export function ReviewSection({ tourId }: { tourId: string }) {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await apiEndpoints.reviews.list(tourId)
      return res.data.results as TourReview[]
    },
  })
  ...
  const deleteMutation = useMutation({
    mutationFn: (reviewId: string) => apiEndpoints.reviews.delete(tourId, reviewId),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['reviews', tourId] })
      queryClient.invalidateQueries({ queryKey: ['tour', tourId] })
      setDeletingId(null)
  ...
  return (
    <section className="space-y-4">
      ...
      <div className="flex items-center justify-between gap-2 flex-wrap">
        ...
        {/* Show "Write a review" only if authenticated and hasn't reviewed yet */}
      ...
      {/* New review form (shown above the list) */}
      ...
      {isLoading ? (
        ...
        <div className="space-y-3">
          {reviews.map((review) =>
            editingId === review.id ? (
              // Edit form renders in-place, replacing the card
              <ReviewForm
                key={review.id}
                tourId={tourId}
                reviewId={review.id}
