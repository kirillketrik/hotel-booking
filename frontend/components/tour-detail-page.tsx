...
import type { Hotel, Tour, TourAvailability, TourTransfer } from '@/lib/types'
import { cn } from '@/lib/utils'

type TourDetailMode = 'public' | 'manage'
...
async function fetchPublicTour(tourId: string): Promise<Tour> {
  const res = await api.get(`/api/v1/tours/${tourId}/`)
  return res.data
}

async function fetchAgencyTour(agencyId: string, tourId: string): Promise<Tour> {
  const res = await apiEndpoints.tours.get(agencyId, tourId)
  return res.data
}
...
function HotelCard({ hotel }: { hotel: Hotel }) {
  ...
  useEffect(() => {
    ...
    const handler = (e: KeyboardEvent) => {
      if (e.key === 'ArrowLeft') {
        setLightboxIndex((i) => ((i ?? 0) - 1 + hotel.images.length) % hotel.images.length)
      }
      if (e.key === 'ArrowRight') {
        setLightboxIndex((i) => ((i ?? 0) + 1) % hotel.images.length)
      }
              ...
              <div key={img.id} className="flex-1 overflow-hidden relative">
                ...
                {i === 1 && hotel.images.length > 3 && (
                  <div className="absolute inset-0 bg-black/50 flex items-center justify-center">
                    <span className="text-white text-xs font-semibold">+{hotel.images.length - 3}</span>
                  </div>
                )}
              </div>
            ))}
        ...
        {hotel.amenities.length > 0 && (
          <div className="flex flex-wrap gap-1.5 mb-4">
            {hotel.amenities.map((a) => (
              <span key={a.id} className="inline-flex items-center gap-1 text-xs bg-muted px-2 py-0.5 rounded-full">
                {a.name}
              </span>
            ))}
          </div>
...
export function TourDetailPage({ mode, agencyId }: TourDetailPageProps) {
  const params = useParams()
  const rawTourId = mode === 'manage' ? params.tourId : params.id
  const tourId = (Array.isArray(rawTourId) ? rawTourId[0] : rawTourId) ?? ''
  ...
  const { data: tour, isLoading } = useQuery({
    queryKey: isManageMode ? ['agency-tour', agencyId, tourId] : ['tour', tourId],
    queryFn: () =>
      isManageMode && agencyId ? fetchAgencyTour(agencyId, tourId) : fetchPublicTour(tourId),
  })
  ...
  const { data: employees } = useQuery({
    ...
    enabled: isManageMode && !!tour?.agency && !!user,
  })
  ...
  const canManageTour = isManageMode && (isStaff() || isMember)
  const canDeleteTour = isManageMode && (isStaff() || myRole === 'owner' || myRole === 'admin')

  const invalidateTourQueries = () => {
    qc.invalidateQueries({ queryKey: ['tour', tourId] })
    qc.invalidateQueries({ queryKey: ['agency-tour', agencyId, tourId] })
  ...
  const detailQueryKey = isManageMode && agencyId ? ['agency-tour', agencyId, tourId] : ['tour', tourId]
    ...
    <PageShell>
      <div className="max-w-6xl mx-auto px-4 py-6">
        <div className="flex items-center gap-2 text-sm text-muted-foreground mb-5">
          ...
          <Link href={`/agencies/${tour.agency}`} className="hover:text-foreground transition-colors">
            {tour.agency_name}
          </Link>
          <ChevronRight className="w-3.5 h-3.5" />
          ...
          <div className="mb-8 rounded-2xl overflow-hidden">
            {allImages.length === 1 ? (
              <div className="h-[420px]">
                <img src={allImages[0].image} alt={tour.title} className="w-full h-full object-cover" />
              </div>
            ) : (
                  ...
                  >
                    ...
                    {i === 3 && allImages.length > 5 && (
                      <div className="absolute inset-0 bg-black/50 flex flex-col items-center justify-center gap-1">
                        ...
                        <span className="text-white text-sm font-semibold">+{allImages.length - 5} more</span>
                      </div>
                    )}
                  </div>
                ))}
        ...
        <div className="grid lg:grid-cols-3 gap-8 items-start">
          <div className="lg:col-span-2 space-y-8">
            <div>
              <div className="flex items-start justify-between gap-3 mb-2">
                ...
                <WishlistButton
                  ...
                  invalidateKeys={[detailQueryKey, ['wishlist']]}
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              ...
              />
              <StatPill icon={<Users className="w-4 h-4" />} label="Adults" value={`Up to ${tour.max_adults}`} />
              <StatPill
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                    {tour.images.map((img, i) => (
                      <div
                        ...
                        onClick={() => {
                          setGalleryIndex(tour.cover_image ? i + 1 : i)
                          setGalleryOpen(true)
                        }}
                      >
                        <img
          ...
          <div className="lg:sticky lg:top-6">
            <div className="rounded-2xl border bg-card shadow-sm p-6 space-y-5">
                  ...
                  <ul className="space-y-1">
                    {includedTransfers.map((t) => (
                      <li key={t.id} className="flex items-center gap-2 text-sm">
                        ...
                        <span>{t.transfer_type.charAt(0).toUpperCase() + t.transfer_type.slice(1)} transfer</span>
                      </li>
                    ))}
                  </ul>
              ...
              <Link href={`/agencies/${tour.agency}`} className="flex items-center justify-between text-sm group">
      ...
      <Dialog open={galleryOpen} onOpenChange={setGalleryOpen}>
        <DialogContent className="max-w-4xl p-2 bg-black border-black">
          <div className="relative">
            <img src={allImages[galleryIndex]?.image} alt="" className="w-full max-h-[80vh] object-contain rounded-lg" />
              ...
              <div className="absolute bottom-3 left-1/2 -translate-x-1/2 flex items-center gap-1.5">
                {allImages.map((_, i) => (
                  <button
                    ...
                    className={cn('w-1.5 h-1.5 rounded-full transition-all', i === galleryIndex ? 'bg-white w-4' : 'bg-white/50')}
                  />
                ))}
              </div>
            )}
